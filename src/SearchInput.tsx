  ...
  return (
    <div className={`relative ${className}`}>
      ...
      <input
        type="search"
        value={value}
        className={`w-full rounded-lg border border-[var(--ui-border)] bg-[var(--ui-surface-hover)] pl-10 pr-${hasValue && onClear ? '9' : '3'} py-2 text-sm text-[var(--ui-text)] outline-none focus:border-[var(--ui-primary)] transition-colors placeholder:text-[var(--ui-text-muted)]`}
        {...props}
      />
      {hasValue && onClear && (
        <button
